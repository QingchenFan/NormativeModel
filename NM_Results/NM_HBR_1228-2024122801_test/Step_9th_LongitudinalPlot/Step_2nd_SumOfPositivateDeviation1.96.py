...
before = pd.read_csv('/Volumes/QCI/NormativeModel/Results/Result_GrayVol246_HBR_HCMDD_1129/StaResults/Longitudinal/'
                     'subtype2_Positive_GreaterThan_196sum_0w.csv')
data2 = list(before['sum_196'])
print(data2)
# 创建一个DataFrame来存储数据
df = pd.DataFrame({'Group': ['After'] * len(data1) + ['Before'] * len(data2), 'Values': data1 + data2})

# 创建图形和轴对象
fig, ax = plt.subplots()

# 设置不同的颜色给两个分组
palette = {'After': '#C17F9E', 'Before': '#8BACD1'}
