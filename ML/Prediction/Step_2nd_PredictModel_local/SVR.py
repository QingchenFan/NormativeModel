...
#    Covariates = data_list[2]  # Todo: Covariates
    ...
    #   Covariates_train, Covariates_test = Covariates[train_index, :], Covariates[test_index, :] # Todo: Covariates
        ...
        # Model
        Lsvr = LinearSVR()
        param_grid = dict(C=[math.pow(2, i) for i in range(-5, 10)])
        cv_time = 2
        predict_model = GridSearchCV(Lsvr, param_grid=param_grid, verbose=1, cv=cv_time)
...
if __name__ == '__main__':
    ...
    for i in range(1,102):
        PLSPrediction_Model(data_list, dimention, weightpath, Permutation=0, kfold=10, datamark='HAMD_SVR', outputdatapath=outputdatapath, count=i, Time=3)